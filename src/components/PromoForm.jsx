...
// Convert datetime-local value to display
...
export default function PromoForm({ promo, onSave, onClose }) {
  ...
  const isPromoCode = form.type === 'Promo Code'

  const handleSubmit = async () => {
    if (!form.domain) return alert('Domain is required')
    if (!form.type) return alert('Promo Type is required')
    ...
    if (isPromoCode && !form.promo_code.trim()) return alert('Promo code is required')
    setSaving(true)
    await onSave(form)
    setSaving(false)
  }

  // Step 1: Domain not selected yet
  const showTypeSelector = !!form.domain && !!form.campaign_objective
  const showFields = showTypeSelector && !!form.type

  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg" style={{ maxWidth: isPromoCode ? '580px' : '680px' }}>
        ...
        <div className="modal-body">
          ...
          {/* STEP 1 — Domain + Objective (always shown) */}
          <div className="section-heading">Context</div>
          ...
          {/* STEP 2 — Promo Type (shown after domain + objective) */}
          {showTypeSelector && (
            <>
              <div className="section-heading">Promo Type</div>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '18px' }}>
                {PROMO_TYPES.map(t => (
                  <button key={t} onClick={() => set('type', t)} style={{
                    padding: '8px 18px', borderRadius: '20px', fontSize: '0.82rem', fontWeight: 600,
                    border: '2px solid', cursor: 'pointer', transition: 'all 0.12s',
                    background: form.type === t ? 'var(--accent)' : 'var(--bg3)',
          ...
          {showFields && (
            <>
              ...
              {isPromoCode && (
                <>
                  ...
                  <div className="form-group">
                    ...
                    <input placeholder="e.g. Happy Hour Free Bet — April" value={form.name} onChange={e => set('name', e.target.value)} />
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <input
                        ...
                        style={{ fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', fontSize: '1rem' }}
                      />
                    </div>
                    <div className="form-group">
                      <label>Reporting Tag *</label>
                      <input
                        placeholder="e.g. TR_PC_HAPPY25_APR"
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <select value={form.reward_type} onChange={e => set('reward_type', e.target.value)}>
                        <option value="">— Select —</option>
                        {REWARD_TYPES_PROMO_CODE.map(r => <option key={r}>{r}</option>)}
                      </select>
                    </div>
                    <div className="form-group">
              ...
              {/* ── OTHER TYPES (Ad-Hoc, Reload, Funnel) — existing full form ── */}
              {!isPromoCode && (
                <>
                  <div className="section-heading">Promotion Details</div>
                  <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <textarea placeholder="e.g. Deposit min ₺200 and get 25% match up to ₺500..." value={form.offer_description} onChange={e => set('offer_description', e.target.value)} />
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                      <label>Reward Type</label>
                  ...
                  <div className="form-group">
                    <label>Special Events / Context</label>
                    <textarea placeholder="e.g. Süper Lig Derby weekend on Day 2..." value={form.event_context} onChange={e => set('event_context', e.target.value)} style={{ minHeight: '60px' }} />
                  </div>
                </>
              )}
            </>
